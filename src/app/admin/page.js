...
const AdminPanel = () => {
  ...
  const handleApprove = async (id, isApproved) => {
    ...
    setBlogs(blogs.map(blog => blog.id === id ? { ...blog, isApproved } : blog));
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8 font-sans scroll-smooth">
        ...
        <>
          ...
          <div className="mb-6 flex gap-4 border-b">
            <button
              ...
              className={`pb-2 px-4 ${activeTab === "all" ? "border-b-2 border-teal-600 text-teal-600" : "text-gray-600"}`}
            >
              All Blogs
            </button>
            <button
              onClick={() => setActiveTab("approved")}
              className={`pb-2 px-4 ${activeTab === "approved" ? "border-b-2 border-teal-600 text-teal-600" : "text-gray-600"}`}
            >
              Approved
            </button>
            <button
              onClick={() => setActiveTab("not-approved")}
              className={`pb-2 px-4 ${activeTab === "not-approved" ? "border-b-2 border-teal-600 text-teal-600" : "text-gray-600"}`}
            >
              Not Approved
            </button>
          </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  {blogs.map(blog => (
                    ...
                    >
                      ...
                      <p className="text-gray-600 text-sm mt-2 line-clamp-3">{blog.subtitle}</p>
      ...
      {/* Edit Modal */}
 Determined that the edit modal is unchanged and should remain as is
      {showModal && (
